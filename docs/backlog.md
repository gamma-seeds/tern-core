...
1. **Threshold sweep on Qwen3-30B-A3B — 0.5 / 0.55 / 0.6 measured, all collapse to single-token repetition; every threshold ≥0.5 collapses by monotonicity.** Three thresholds were swept on Qwen3-30B-A3B and each lands below the coherent-generation envelope under pure ternary quantisation: 0.5 → one unique token, 0.55 → two, 0.6 → two. Threshold 0.5 is the least-aggressive setting swept and already collapses; raising the threshold only sparsifies further, so every threshold above 0.5 — 0.7 included — collapses too. The coherent-generation envelope therefore sits below 0.5, and the ≥0.7 region follows by monotonicity rather than by separate enumeration. A dense reference confirms the envelope holds at 0.7 directly: Phi-4-14B ternary @0.7 collapse is independently documented (finding 6 below). Evidence-of-record holds exactly the three measured points — `threshold_coherence_results.json` (0.5 / 0.55 / 0.6) on the private `gamma-seeds/ecc-ternary` release `evidence-tern-core-scans-2026-05-28` (off-site, access-gated) — https://github.com/gamma-seeds/ecc-ternary/releases/download/evidence-tern-core-scans-2026-05-28/threshold_coherence_results.json . Driver: `benchmarks/sweep_qwen3_threshold_coherence_2026-05-28.py` (extend its `thresholds` list to enumerate a measured 0.7 point on Qwen3 should direct measurement ever be wanted over the monotonicity inference).
...
2. **Per-tensor sensitivity scan (Qwen3 + Gemma-4-26B-A4B reference) confirms a smeared distribution.** Std ~0.01, errors clustered at relative_error ~0.45 across 18,624 Qwen3 layers and 7,885 Gemma-4 layers; no concentrated tail (Qwen3: 13 layers ≥0.54, 0 ≥0.60; Gemma-4: 5 layers ≥0.54, 0 ≥0.60). Per-layer INT4 routing is the wrong lever against a smeared distribution. Evidence:
   - `sensitivity_scan_qwen3-30b-a3b_2026-05-28.json` (149,221 lines) — https://github.com/gamma-seeds/ecc-ternary/releases/download/evidence-tern-core-scans-2026-05-28/sensitivity_scan_qwen3-30b-a3b_2026-05-28.json
   - `sensitivity_scan_gemma4-26b-a4b_2026-05-28.json` (63,317 lines) — https://github.com/gamma-seeds/ecc-ternary/releases/download/evidence-tern-core-scans-2026-05-28/sensitivity_scan_gemma4-26b-a4b_2026-05-28.json
   - Evidence-of-record: private `gamma-seeds/ecc-ternary` release `evidence-tern-core-scans-2026-05-28` (off-site, access-gated; SHA-256 in release MANIFEST). Driver: `benchmarks/sensitivity_scan_moe_2026-05-28.py` (regenerates both dumps).
...
**Round-2 measurements (2026-05-28, concluded):** both cheap probes returned *confirm-not-rescue*, hardening the smeared verdict.

5. **Per-channel α is not the lever — it is worse.** Computing reconstruction error with α per output channel (instead of per tensor) at threshold 0.7 shifts the whole distribution upward, not just its tail:
   ...
   - Evidence-of-record (private `gamma-seeds/ecc-ternary` release `evidence-tern-core-scans-2026-05-28`, off-site + access-gated; regenerable via `benchmarks/sensitivity_scan_moe_2026-05-28.py`): `sensitivity_scan_qwen3-30b-a3b_per_channel_2026-05-28.json` (223,720 lines) https://github.com/gamma-seeds/ecc-ternary/releases/download/evidence-tern-core-scans-2026-05-28/sensitivity_scan_qwen3-30b-a3b_per_channel_2026-05-28.json , `sensitivity_scan_gemma4-26b-a4b_per_channel_2026-05-28.json` (94,860 lines) https://github.com/gamma-seeds/ecc-ternary/releases/download/evidence-tern-core-scans-2026-05-28/sensitivity_scan_gemma4-26b-a4b_per_channel_2026-05-28.json .

6. **Dense reference (Phi-4-14B) shows the same smeared shape.** Per-tensor scan on Phi-4: mean 0.5447, p99 0.6304, max 0.6576; 53 of 160 layers ≥0.54, 6 ≥0.60. Tightly clustered, same structural pattern as the MoE models. Attention is uniformly the worst category across all three architectures (Phi-4 attn mean 0.559 vs MLP 0.531; Qwen3 attn 0.494 vs expert 0.447; Gemma-4 attn 0.495 vs other 0.453). Phi-4 ternary @0.7 collapse (already documented) sits in the same numerical neighbourhood, confirming the smeared pattern is a general property of ternary @0.7 on transformer architectures, **not** Qwen3-specific or MoE-specific. Evidence-of-record: `sensitivity_scan_phi-4_2026-05-28.json` on the private `gamma-seeds/ecc-ternary` release `evidence-tern-core-scans-2026-05-28` (off-site, access-gated) — https://github.com/gamma-seeds/ecc-ternary/releases/download/evidence-tern-core-scans-2026-05-28/sensitivity_scan_phi-4_2026-05-28.json

7. **Implication for the Mistral-7B 297.6 tok/s headline:** Mistral-7B sits in the same dense-transformer class as Phi-4. Its coherence was likewise never gated at compression time (`fidelity_pass` only checks layer count). The headline figure is a throughput measurement on a model whose generation coherence is unverified. Worth knowing before it surfaces in partner conversations.

**Position on M1 deliverables:** the Milestone-1 inference machinery (`terncore.moe` bank loader + custom block + lifecycle, PRs #40/#41/#42) is independent of this finding and verified working. P145/P146 patent footing intact. Compression-quality is **a research workstream, not a contained patch** — per-channel and lower-threshold both fail to rescue; the deeper interventions (outlier protection, AWQ/SqueezeLLM-style calibration, substantially-more-INT4 mixed precision, QAT fine-tuning, or larger-than-ternary precision floors) are out-of-scope of this PR stack and need their own scoping.
