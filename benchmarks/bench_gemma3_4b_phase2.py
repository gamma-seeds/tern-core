#!/usr/bin/env python3
"""
bench_gemma3_4b_phase2.py — Apple brief benchmark for Llama 3.2 3B ternary on M4 Pro.
...
MODEL_PATH = Path(
    "/Users/syn/synapticode/models/coreml/gemma3-4b"
    "/gemma3_4b_ternary_v0.1.0.mlpackage"
)
PALETTISED_PATH = Path(
    "/Users/syn/synapticode/models/coreml/gemma3-4b"
    "/gemma3_4b_ternary_v0.1.0_2bit.mlpackage"
)
RESULTS_PATH = Path(__file__).parent / "gemma3_4b_phase2.json"
...
def main() -> int:
    report = {
        "runner": "bench_gemma3_4b_phase2.py",
    ...
    try:
        # ── spec ──
        print("=" * 72, flush=True)
        print("  Gemma 3 4B Ternary — Apple Brief Benchmark", flush=True)
        print("=" * 72, flush=True)
        ...
        # ── summary ──
        print(f"\n{'='*72}", flush=True)
        print("  RESULTS — Gemma 3 4B Ternary (Apple Brief)", flush=True)
        print(f"{'='*72}", flush=True)
