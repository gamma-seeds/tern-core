#!/usr/bin/env python3
"""
bench_dsr1_14b_phase2.py — Apple brief benchmark for DeepSeek-R1 Distil 14B ternary on M4 Pro.

Produces: tokens/sec, peak RSS, energy profile across compute units.
Tests both the raw ternary-as-INT4 mlpackage and a 2-bit palettised
...
MODEL_PATH = Path(
    "/Users/syn/synapticode/models/coreml/dsr1-14b"
    "/dsr1_14b_ternary_v0.1.0.mlpackage"
)
PALETTISED_PATH = Path(
    "/Users/syn/synapticode/models/coreml/dsr1-14b"
    "/dsr1_14b_ternary_v0.1.0_2bit.mlpackage"
)
RESULTS_PATH = Path(__file__).parent / "dsr1_14b_phase2.json"
...
WATCHDOG_COMPRESSOR_TRIP_PAGES = 3_000_000  # raised from 1.8M for 64 GB M4 Pro
WATCHDOG_POLL_S = 3.0
...
# ── load timeout guard ───────────────────────────────────────────────
# DSR1-7B observed a 9,176s ANE compile for 2bit_CPU_AND_NE.
# Skip the compute unit if a subprocess probe exceeds this budget.
PALETTISED_NE_LOAD_TIMEOUT_S = 60.0


def probe_load_timeout(model_path: Path, cu_label: str,
                       timeout_s: float) -> tuple[bool, float]:
    """Fork a subprocess to attempt model load. Kill on timeout."""
    script = (
        "import sys, time, coremltools as ct\n"
        "t0=time.perf_counter()\n"
        f"m=ct.models.MLModel(sys.argv[1], compute_units=ct.ComputeUnit.{cu_label})\n"
        "print(f'ok {time.perf_counter()-t0:.2f}', flush=True)\n"
    )
    t0 = time.perf_counter()
    try:
        r = subprocess.run(
        ...
        return (r.returncode == 0, time.perf_counter() - t0)
...
def main() -> int:
    report = {
        "runner": "bench_gemma3_12b_phase2.py",
    ...
    try:
        # ── spec ──
        print("=" * 72, flush=True)
        print("  DeepSeek-R1 7B Ternary — Apple Brief Benchmark", flush=True)
        print("=" * 72, flush=True)
        ...
        try:
            ...
            for cu_name, cu in COMPUTE_UNITS:
                if cu_name == "CPU_AND_NE":
                    print(f"  [2bit_{cu_name}] probing load "
                          f"(≤{PALETTISED_NE_LOAD_TIMEOUT_S:.0f}s "
                          f"guard — DSR1-7B saw 9,176s regression)",
                          flush=True)
                    ok, probe_s = probe_load_timeout(
                        PALETTISED_PATH, "CPU_AND_NE",
                        PALETTISED_NE_LOAD_TIMEOUT_S,
                    )
                    if not ok:
                        print(f"  [2bit_{cu_name}] SKIPPED "
                              f"(probe exceeded budget, "
                              f"elapsed={probe_s:.1f}s)", flush=True)
                        report["palettised_benchmarks"][cu_name] = {
                            "compute_units": f"2bit_{cu_name}",
                            "skipped": True,
                            "reason": "load_timeout_guard",
                            "probe_elapsed_s": probe_s,
                            "timeout_budget_s":
                                PALETTISED_NE_LOAD_TIMEOUT_S,
                        }
                        dump()
                        continue
                    print(f"  [2bit_{cu_name}] probe ok "
                          f"({probe_s:.1f}s), proceeding",
                          flush=True)
                r = benchmark_cu(
        ...
        if sudo_available():
            ...
            pal_measured = [r for r in
                            report["palettised_benchmarks"].values()
                            if not r.get("skipped")]
            if phase_c_ok and pal_measured:
                best_pal = min(
                    pal_measured,
                    key=lambda r: r["latency_mean_ms"],
                )
        ...
        # ── summary ──
        print(f"\n{'='*72}", flush=True)
        print("  RESULTS — DeepSeek-R1 14B Ternary (Apple Brief)", flush=True)
        print(f"{'='*72}", flush=True)
        ...
        for label, bm in [("Raw FP16-tern", report["raw_benchmarks"]),
            ...
            for cu, r in bm.items():
                tag = f"{label} ({cu})"
                if r.get("skipped"):
                    print(f"  {tag:<25} SKIPPED — {r.get('reason','')}",
                          flush=True)
                    continue
